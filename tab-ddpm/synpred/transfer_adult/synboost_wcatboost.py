"""
Syn-Boost tuning with the fine-tuned generator on Adult-Female, which was originally pre-trained on Adult-Male.
"""


import os
os.environ["REPO_DIR"] = "/home/lw764/pizhao/syn/"
REPO_DIR = os.environ.get("REPO_DIR")
TDDPM_DIR = os.path.join(REPO_DIR, "tab-ddpm")

import numpy as np
import pandas as pd
from tqdm import tqdm
import pickle

import sys
sys.path.insert(0, os.path.join(TDDPM_DIR, "utils/"))

from utils_tabddpm import (
    generate_sample,
)

from utils_syn import (
    concat_data,
    catboost_pred_model,
    test_rmse,
    test_acc,
)



######## Configurations ########

n = 1350
rho_max, num_rhos = 30, 30
#rho_list = np.linspace(0, rho_max, num_rhos + 1)[1:]
#rho_list = np.arange(0.1, 1.0, 0.1)
rho_list = [5]
rho_max=5
twin_id=1
save_path = f"synboost_transfer_adult_female_twin{twin_id}_rho5.pkl"

df_dict = pickle.load(open("temp_df_dict.pkl", "rb"))
gender_names_dict = df_dict["gender_names_dict"]

# Get the test female data (twin_2 or twin_1 test) as the validation set
df_female_test = concat_data(
    f"{TDDPM_DIR}/data/adult_female_3000_twin_{twin_id}",
    split="test",
    **gender_names_dict,
)


######### Generate synthetic female data (from male) #########
synthetic_sample_dir = generate_sample(
    pipeline_config_path=f"{TDDPM_DIR}/exp/adult_male/ddpm_cb_best/config.toml",
    ckpt_path=f"{TDDPM_DIR}/exp/adult_male/ddpm_cb_best/model.pt",
    pipeline_dict_path=f"{TDDPM_DIR}/exp/adult_male/ddpm_cb_best/pipeline_dict.joblib",
    num_samples=int(n * rho_max),
    batch_size=int(n * rho_max / 10),
)
fake_female_df_pt_allinone = concat_data(synthetic_sample_dir, **gender_names_dict)
fake_female_df_pt_allinone["income"].cat.categories = ["0", "1"]
fake_df_train_pt = fake_female_df_pt_allinone.copy()


df_test = df_female_test.copy()

######### Generate synthetic female data (fine-tuned generator) #########
#synthetic_sample_dir = generate_sample(
#    pipeline_config_path=f"{TDDPM_DIR}/exp/adult_female_3000_twin_{twin_id}/ddpm_cb_best/config.toml",
#    ckpt_path=f"{TDDPM_DIR}/exp/adult_female_3000_twin_{twin_id}/ddpm_cb_best/model.pt",
#    pipeline_dict_path=f"{TDDPM_DIR}/exp/adult_female_3000_twin_{twin_id}/ddpm_cb_best/pipeline_dict.joblib",
#    num_samples=int(n * rho_max),
#    batch_size=int(n * rho_max / 10),
#    seed=2023,
#)
#fake_df_female_allinone = concat_data(synthetic_sample_dir, **gender_names_dict)
#fake_df_female_allinone["income"].cat.categories = ["0", "1"]
#
#
#fake_df_train = fake_df_female_allinone.copy()
#
#
########### Syn-Boost tuning ##########
#
#result_dict = {"rhos": rho_list, "scores": []}
#for rho in tqdm(rho_list):
#    m = int(n * rho)
#
#    fake_train_df = fake_df_train[:m]
#
#
#    cat_model = catboost_pred_model(
#        fake_train_df,
#        df_test,
#        **gender_names_dict,
#        iterations=1000,
#        verbose=False
#    )
#
#    score = test_acc(cat_model, df_test, **gender_names_dict)
#    result_dict["scores"].append(score)
#
#    pickle.dump(result_dict, open(save_path, "wb"))
#    print(f"rho = {rho}, m = {m}: Test accuracy is {score}.")
#    
#
#print(f"Syn-Boost tuning is done. The result is saved in {save_path}.")
#
########## Syn-Boost tuning (combined) ##########

df_female_raw = concat_data(
    f"{TDDPM_DIR}/data/adult_female_3000_twin_{twin_id}",
    split="train",
    **gender_names_dict,
)

save_path = f"synboost_transfer_adult_female_combined_from_male_twin{twin_id}_rho5.pkl"
weight_list = np.arange(0.1, 2.0, 0.1)
result_dict = {"weights": weight_list, "scores": []}
rho = 5
for fake_weight in tqdm(weight_list):
    m = int(n * rho)

    fake_train_df0 = fake_df_train_pt[:m]
    fake_train_df = pd.concat([df_female_raw, fake_train_df0], axis=0, ignore_index=True)

    cat_model = catboost_pred_model(
        fake_train_df,
        df_test,
        **gender_names_dict,
        iterations=1000,
        verbose=False,
        nfake = fake_train_df.shape[0],
        fake_weight = fake_weight
    )

    score = test_acc(cat_model, df_test, **gender_names_dict)
    result_dict["scores"].append(score)

    pickle.dump(result_dict, open(save_path, "wb"))
    print(f"fake_weight = {fake_weight}, m = {m}: Test accuracy is {score}.")


print(f"Syn-Boost tuning (combined) is done. The result is saved in {save_path}.")

