...
data_folder = f"mydata_toCombine{args.arg6}_{args.arg2}_{args.arg3}_{args.arg4}"
...
#rho_list = np.linspace(rho_min, rho_max, int((rho_max - rho_min) / step_size) + 2)
#
...
for rho in tqdm(rho_list):
    ...
    fake_train_df0 = concat_data(temp_dir, split="train")
    fake_train_df = pd.concat([train_df, fake_train_df0], axis=0, ignore_index=True)
    ...
    pickle.dump(result_dict, open(f"./results_{data_folder}/{keyword}_combined.pkl", "wb"))
    print(f"rho = {rho}, m = {m}: Test RMSE is {score}.")
