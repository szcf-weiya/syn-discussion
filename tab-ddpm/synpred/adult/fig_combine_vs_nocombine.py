...
ax.plot(ratios[1:], 1 - np.array(scores_combined[1:]), label = "Syn-Boost (combine synthetic and raw)", marker = "s", ms = 10)
ax.plot(ratios[1:], 1 - np.array(scores_nocombine[1:]), label = "Syn-Boost (only synthetic)", marker = "^", ms = 10)
...
plt.savefig("adult-combine-vs-nocombine.pdf", bbox_inches='tight')