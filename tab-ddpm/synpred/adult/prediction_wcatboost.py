...
import lib
from lib import concat_features, read_pure_data, get_catboost_config, read_changed_val, load_json
...
parser.add_argument("--dsname", type=str, default="adult")

# parser.add_argument("--metric", type=str, default="rmse")
# whether to use alternative metric for evaluation: "r2" to replace "rmse" and "f1" to replace "acc"

parser.add_argument("--fakeratio", type=int, default=10, help="one fake-to-real ratio")
parser.add_argument("--fakeweightstep", type=float, default=0.2, help="one fake-to-real ratio")
...
#fake_to_real_ratio_list = np.linspace(0.1, 0.9, 9)
fake_to_real_ratio_list = [10]
...
def train_catboost_simple(
    ...
    sample_weight = np.r_[np.ones(n1), np.ones(n2) * fake_weight]
    model.fit(X["train"], D.y["train"], eval_set=(X["val"], D.y["val"]), verbose=100, sample_weight=sample_weight)
...
def objective(fake_to_real_ratio=0, fake_weight=1, add_original_train=False):
    # params defines the hyparameters to be tuned for the catboost model
    # params = suggest_catboost_params(trial)
#    params = None #then it wil load the saved _cv.json config (required relative path)
    params = load_json(f'../tuned_models/catboost/adult_cv.json')
...
if __name__ == "__main__":
    study_list = []
    scores = []
    #fake_weight_list = [0.1, 0.2, 0.5]
    step = args.fakeweightstep
    #fake_weight_list = np.arange(0.1, 1.9, step)
    fake_weight_list = 10 ** np.arange(-3, 0.3, step)

    nfakeweight = len(fake_weight_list)
    fake_ratio = args.fakeratio
    save_path_score = f"scores_ratio{fake_ratio}_fakeweight{step}_log.pkl"
    save_path_weight = f"weights_ratio{fake_ratio}_fakeweight{step}_log.pkl"
    for fake_weight in fake_weight_list:
...
#        )
#
        scores.append(objective(fake_to_real_ratio=fake_ratio, fake_weight=fake_weight))
        #fake_to_real_ratio_list_raw.append(fake_to_real_ratio)
        
    pickle.dump(fake_weight_list, open(save_path_weight, "wb"))
    pickle.dump(scores, open(save_path_score, "wb"))
